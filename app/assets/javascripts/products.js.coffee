...
$ ->
  ...
  $('.add_to_cart').click ->
    ...
    if $.cookie("cart_items") == "" || $.cookie("cart_items") == null
      $.cookie("cart_items", $(this).data("productid"))
      ...
      if val.search(new_product) != -1
        ...
        $.cookie("cart_items", $.cookie("cart_items") + "," + $(this).data("productid"))
        cart_button.attr("disabled", true)
        $('#cartprice').text(parseInt($('#cartprice').text()) + $(this).data("product-price"))
        $('#cartqty').text(parseInt($("#cartqty").text()) + 1)
    $.cookie("total_price", $('#cartprice').text())
    $.cookie("total_items", $('#cartqty').text())
  ...
  $('.delete_from_cart').click ->
    ...
    $.cookie("cart_items", cart_products.join(","))
    $.cookie("total_price", $('#total-price').text())
    $.cookie("total_items", $('#cartqty').text())
